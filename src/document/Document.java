...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for (int i = 0; i < ch.length; i++) {
			if (i == ch.length-1 && Character.toLowerCase(ch[i]) == 'e' 
			    		&& newSyllable && count > 0) {
	            count--;
	        }
			if (newSyllable && vowels.indexOf(Character.toLowerCase(ch[i])) >= 0) {
				newSyllable = false;
				count++;
			}
			else if (vowels.indexOf(Character.toLowerCase(ch[i])) < 0) {
				newSyllable = true;
			}
			}
		return count;
	...
	public double getFleschScore()
	{
		...
		int sents = getNumSentences();
		int syllbs = getNumSyllables();
		double score = 206.835 - 1.015*((double)words/sents) - 84.6*((double)syllbs/words);
	    return score;
	}
